...
interface Decision {
  ...
  topic: string;
  value: string;
  supersedes?: string;   // ID keputusan lama yang digantikan
  timestamp: number;
  active: boolean;       // false jika sudah di-supersede
}
...
interface EvalMetrics {
  ...
  retrievalMentioned: number;
  retrievalUtilized: number;
  workspaceRetrieved: number;
...
export class Agent {
  ...
  private workspaceIndex: WorkspaceFile[] = [];
  private workspaceIndexBuilt = false;

  // Token Budget Limits (Balanced for 32k context window)
  private softLimit = 24000;
  private hardLimit = 32000;
  ...
  private lastRetrievedMemories: MemoryEntry[] = [];
  private lastRetrievedWorkspaceFiles: string[] = [];
  private lastRetrievedMemoriesText = '';
  private lastRetrievedWorkspaceText = '';
  private workspaceOpenedThisTurn = new Set<string>();
  private workspaceUsedThisTurn = new Set<string>();
  ...
  private buildWorkspaceIndex(): void {
    ...
    const indexFile = (filePath: string) => {
  ...
  private retrieveWorkspaceFiles(queryWords: string[]): string {
    if (!this.workspaceIndexBuilt) this.buildWorkspaceIndex();
    ...
    const scored = this.workspaceIndex.map(file => {
      ...
      file.tags.forEach(tag => {
        if (queryWords.includes(tag)) score += 2;
      });
      const pathWords = file.path.toLowerCase().split(/[/\-_.]/);
      queryWords.forEach(qw => {
        if (pathWords.includes(qw)) score += 1;
      });
      return { file, score };
    });
  ...
  private addOrUpdateDecision(topic: string, value: string): void {
    const normalizedTopic = topic.toLowerCase().trim();

    const existing = this.activeDecisions.find(
      d => d.active && d.topic.toLowerCase().trim() === normalizedTopic
    );
...
---`;
  ...
  private checkRuleBasedViolations(output: string, toolCalls: any[] = []): { violatedConstraints: string[], violatedDecisions: string[] } {
    const violatedConstraints: string[] = [];
    const violatedDecisions: string[] = [];
  ...
  private saveViolationSnapshot(userInput: string, output: string, violations: string[]): void {
    try {
  ...
  private async summarizeLargeToolOutput(toolName: string, output: string): Promise<string> {
...
${output.substring(0, 80000)} ... (sisa output dipotong)`
      ...
      if (!summaryRes.ok) throw new Error('Koneksi server gagal');
...
... [Sisa output ${output.length - 8000} karakter dipotong karena terlalu besar]`;
  ...
  private retrieveMemories(userMessage: string): string {
    ...
    const scoreEntry = (entry: MemoryEntry) => {
      let matchScore = 0;

      entry.tags.forEach(tag => {
        if (queryWords.includes(tag.toLowerCase())) matchScore += 3;
      });
      entry.keywords.forEach(kw => {
        if (queryWords.includes(kw.toLowerCase())) matchScore += 1.5;
      });
      const contentWords = cleanAndSplit(entry.content);
      queryWords.forEach(qw => {
        if (contentWords.includes(qw)) matchScore += 0.5;
      });
    ...
    for (const match of scoredEntries) {
      ...
      for (const tag of entry.tags) {
        ...
        if (count >= maxPerTag) { tagLimitReached = true; break; }
      }
      if (tagLimitReached) continue;
    ...
    for (const entry of selectedEntries) {
      ...
      if (currentTokens + entryTokens > budgetLimitTokens) break;
  ...
  private async runDynamicSummarization(lastUserInput: string, lastResponse: string, lastToolCalls: any[] = []): Promise<void> {
...
Tugas Anda adalah mengekstrak memori baru ke dalam JSON valid. Format respons Anda HARUS berupa JSON valid tanpa penjelasan tambahan:
{
  ...
  "retrievedMemoriesMentioned": ["ID dari RETRIEVED MEMORIES di atas yang sempat disebutkan dalam teks respon tetapi tidak benar-benar memandu keputusan atau penulisan kode."]
      ...
      if (!summaryRes.ok) throw new Error('Gagal menghubungi server untuk memadatkan memori');
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          const accumulateList = (oldList: string[], newList: string[]) => {
          ...
          if (parsed.activeDecisions && Array.isArray(parsed.activeDecisions)) {
            for (const dec of parsed.activeDecisions) {
              if (dec.topic && dec.value) {
                const existing = this.activeDecisions.find(
                  d => d.active && d.topic.toLowerCase().trim() === dec.topic.toLowerCase().trim()
                );
          ...
          if (parsed.newEpisodes && Array.isArray(parsed.newEpisodes)) {
            parsed.newEpisodes.forEach((ep: any) => {
          ...
          const llmUtilized = new Set<string>(parsed.retrievedMemoriesUtilized || []);
          const llmMentioned = new Set<string>(parsed.retrievedMemoriesMentioned || []);
...
WORKSPACE INDEX: ${this.workspaceIndex.length} files indexed`;
  ...
  getDashboard(): string {
    ...
    const progressBar = (percentage: number, char = '█', size = 20) => {
    ...
    const getPercent = (val: number) => Math.round((val / totalPrompt) * 100);
    
    const barSegment = (label: string, value: number, colorFn: any) => {
  ...
  private showDiff(diffPath: string, newContent: string) {
    let oldContent = '';
    try {
